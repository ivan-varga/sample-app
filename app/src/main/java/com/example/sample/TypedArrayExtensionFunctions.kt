...
inline fun TypedArray.withReference(recyclerViewPositionIndicatorDrawable: Int, block: (Drawable) -> Unit) {
    getDrawable(recyclerViewPositionIndicatorDrawable)?.let {
        block(it)
    }
}